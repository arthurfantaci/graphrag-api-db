...
def build_cross_reference_graph(index: ArticleIndex) -> dict[str, list[str]]:
    """Build cross-reference graph from article index.

    Creates an adjacency list representing which articles
    reference which other articles.

    Args:
        index: ArticleIndex with cross_references populated.

    Returns:
        Mapping from article_id to list of referenced article_ids.
    """
    graph: dict[str, list[str]] = {}

    for article_id, article_data in index.by_id.items():
        graph[article_id] = []

        # Check cross-references for internal links
        for ref in article_data.get("cross_references", []):
            if ref.get("is_internal"):
                target_url = ref.get("url", "")
                target_id = index.by_url.get(target_url)
                if target_id and target_id != article_id:
                    graph[article_id].append(target_id)

    return graph


def compute_article_stats(index: ArticleIndex) -> dict[str, Any]:
    """Compute statistics from article index.

    Args:
        index: ArticleIndex to analyze.
    ...
    for article_data in index.by_id.values():
        total_words += article_data.get("word_count", 0)
        total_chars += article_data.get("char_count", 0)
        total_sections += article_data.get("section_count", 0)
        total_images += article_data.get("image_count", 0)
        total_videos += article_data.get("video_count", 0)
        total_webinars += article_data.get("webinar_count", 0)
        total_refs += article_data.get("cross_reference_count", 0)

    return {
        "total_articles": index.total_articles,
        "total_chapters": len(index.by_chapter),
        ...
        "avg_words_per_article": total_words // max(index.total_articles, 1),
        "avg_sections_per_article": total_sections // max(index.total_articles, 1),
    }
